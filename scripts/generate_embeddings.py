"""Reads reviews from SQLite, builds one combined
passage per review, embeds them, and upserts into the Chroma vector store.

Run from the repo root:
    python -m scripts.generate_embeddings
    python -m scripts.generate_embeddings --rebuild --limit 50
"""
...
def build_document(row: sqlite3.Row):
    '''Build a LangChain Document from a review row, combining text and metadata.'''
